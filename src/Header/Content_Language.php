 ...
 */
class Content_Language implements Header_Interface
{
    /** @var string */
    protected $value;
    /**
    ...
    public static function from_string($header_line): static
    {
        ...
        if (strtolower($name) !== 'content-language') {
            throw new Exception\InvalidArgumentException(sprintf('Invalid header line for Content-Language string: "%s"', $name));
        }
    ...
    {
        return 'Content-Language: ' . $this->get_field_value();
    }
}