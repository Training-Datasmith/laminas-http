 ...
 */
class Proxy_Authorization implements Header_Interface
{
    /** @var string */
    protected $value;
    /**
    ...
    public static function from_string($header_line): static
    {
        ...
        if (strtolower($name) !== 'proxy-authorization') {
            throw new Exception\InvalidArgumentException(sprintf('Invalid header line for Proxy-Authorization string: "%s"', $name));
        }
    ...
    {
        return 'Proxy-Authorization: ' . $this->get_field_value();
    }
}